local Settings = {
    ["Autorun Commands"] = {"antikick","antiservercrash","antiabuse","antiname","nokill","fixvelocity","antivoid","antifly","autogod","platform","crashdetector","afkindicator","antiskydive","antigrav","antiepilepsy","antilighting","nethelper","serverdata","antispeed","blacklisttools DaggerOfShatteredDimensions;BlackHoleSword;HotDogOnAStick;AzureDragonMagicSlayerSword"}, -- Commands to run automatically
    ["Default Whitelisted"] = {"3cxos","dawninja21alt"}, -- People whitelisted by default
    ...
    ["Script Name"] = "HydraV2", -- Change the name of the script, default "ii' s St upid Ad min"
...
addcommand("lock",{"player"},function(args)
    for i,v in pairs(GetPlayers(args[1])) do
        if not table.find(lock, v.Name) then
        table.insert(lock, v.Name)
        Loops.lock = true
        repeat task.wait()
        if not game.Lighting:FindFirstChild(v.Name) then
            chat("name " .. locker .. " [Hydra]\n LOCKED")
            chat("clone " .. locker)
            chat("trip " .. locker)
            chat("punish " .. locker)
            chat("blind " .. locker)
...
addcommand("prsucks",{},function()
GUI:SendMessage(ScriptName, "Pr Script Sucks.")
end)
...
addcommand("cxo",{"player"},function(args)
    local player = GetPlayers(args[1])
    for i,v in pairs(player) do
        chat("char "..v.Name.." 3222470105")
    end
end)
...
addcommand("pmkick",{"player"},function(args)
    for i,v in pairs(GetPlayers(args[1])) do
        Loops.pmkick = true
        repeat task.wait()
      chat("pm "..v.Name.." 😀😃😄😁😆😅😂🤣☺️😇🙂🙃😉😍😘😗😙😋😛😝😜🤪🤨🧐🤓😎🤩😏😒😞😔😟😕🙁☹️😣😖😫😩😢😭😤😠😡🤬🤯😳😱😨😰😥😓🤗🤔🤭🤫🤥😶😐😑😬🙄😯😦😧😮😲😴🤤😪😵🤐🤢🤮🤧😷🤒🤕🤑🤠😈👿👹👺🤡💩👻💀☠️👽👾🤖🎃😺😸😹😻😼😽🙀😿😾")
      chat("freeze "..v.Name)
      chat("rainbowify "..v.Name)
      chat("name "..v.Name.." Hydra\n Currently getting kicked.")
until not Loops.pmkick
end
end)

addcommand("unpmkick",{"player"},function(args)
for i,v in pairs(GetPlayers(args[1])) do
    Loops.pmkick = false
end
end)
...
addcommand("iy",{},function()
    execute("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source")
end)

addcommand("prscript",{},function()
    loadstring("https://raw.githubusercontent.com/S-PScripts/scripts/main/Luau/KAH/People's%20Scripts/PR_Script.txt")
end)

addcommand("gearblacklist",{"player"},function(args)
for i,v in pairs(GetPlayers(args[1])) do
    if v.Character:FindFirstChild(blacklistedgears) or v.Backpack:FindFirstChild(blacklistedgears) then
    chat("ungear/"..v.Name)
    chat("h\n\n\n\n\n\n\n\n\n\n "..v.Name.."\n Has Just Tried To Use A Blacklisted Gear!")
    end
end
end)
